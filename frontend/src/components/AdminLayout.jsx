    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-gray-200/80 px-4 md:px-6 h-16 flex items-center gap-2 md:gap-4 shadow-soft">
        ...
        <div className="flex-1 flex items-center justify-center gap-3 text-sm px-2 md:px-4">
          {schoolLogo ? (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50/80 rounded-full">
              <img src={schoolLogo} alt="School logo" className="h-5 w-5 object-contain rounded" />
              <span className="text-gray-700 font-medium truncate max-w-32">{schoolName || ''}</span>
            </div>
          ) : (
            <span className="text-gray-600 truncate">{schoolName || ''}</span>
          )}
          {currentTerm && currentYear && (
            <div className="px-3 py-1.5 bg-brand-50 text-brand-700 rounded-full text-xs font-medium border border-brand-200">
        ...
        <div className="ml-auto flex items-center gap-3">
          {user && (
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-full">
              ...
              <span className="text-sm text-gray-700 font-medium">{user.first_name || user.username}</span>
            </div>
          )}
          <button onClick={logout} className="px-4 py-2 rounded-lg text-sm font-medium bg-gray-900 text-white hover:bg-gray-800 transition-all duration-200 shadow-soft">
            Logout
          </button>
        </div>
      </header>
